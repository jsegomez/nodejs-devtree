...
import { BadRequestException, ConflictException, ImageUploadException, InternalServerErrorException, NotFoundException, UnauthorizedException } from "../utils/exceptions/exceptions";
...
export const getPublicDataUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { username } = req.params;
    const user = await User.findOne({ username }).select('-password -createdAt -updatedAt -_id -position');
